...
public final class LifeCycleRetriever implements Handler.Callback {
    ...
    private final LifeCycle mApplicationLifeCycle = new ApplicationLifeCycle();
    ...
    public static LifeCycle with(Context context) {
        return getInstance().get(context);
    }

    public static LifeCycle with(Fragment context) {
        return getInstance().get(context);
    }

    public static LifeCycle with(android.app.Fragment context) {
        return getInstance().get(context);
    }

    public static LifeCycle with(FragmentActivity context) {
        return getInstance().get(context);
    }

    public static LifeCycle with(Activity context) {
        return getInstance().get(context);
    }

    private LifeCycle get(Context context) {
    ...
    private LifeCycle get(Activity activity) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB) {
            return get(activity.getApplicationContext());
        }
        assertNotDestroyed(activity);
        android.app.FragmentManager fm = activity.getFragmentManager();
        return getLifeCycleFragment(fm, null).getLifecycle();
    }

    private LifeCycle get(FragmentActivity activity) {
        assertNotDestroyed(activity);
        android.support.v4.app.FragmentManager fm = activity.getSupportFragmentManager();
        return getSupportLifeCycleFragment(fm, null).getLifecycle();
    }

    private LifeCycle get(android.app.Fragment fragment) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR1) {
            return get(fragment.getActivity());
        }
        android.app.FragmentManager fm = fragment.getChildFragmentManager();
        return getLifeCycleFragment(fm, fragment).getLifecycle();
    }

    private LifeCycle get(Fragment fragment) {
        if (fragment.getActivity() == null) {
            throw new IllegalArgumentException(
                    "You cannot start a load on a fragment before it is attached");
        }
        android.support.v4.app.FragmentManager fm = fragment.getChildFragmentManager();
        return getSupportLifeCycleFragment(fm, fragment).getLifecycle();
    }
    ...
    final SupportLifeCycleFragment getSupportLifeCycleFragment(android.support.v4.app.FragmentManager fm, Fragment parentHint) {
